...
def admin_login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, password):
            login_user(user, remember=True)
            flash(f'Welcome to your page, {user.username}.', 'success')
            return redirect(url_for('profile_bp.get_users_profiles'))
        flash('Oops! email and password error. Please supply your correct detail.', 'danger')
    return render_template('login.html', form=form)

@logbp.route('/logout', methods=['GET'])
@login_required
def logout():
    logout_user()
    current_user.is_aithenticated = False
    return redirect(url_for('appomsbp.home'))
